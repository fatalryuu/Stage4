...
const Dropdown = ({ info, isOpened, onDropdownOpen, onDropdownClose }) => {
    ...
    const configureLinks = (text, index) => {
        if ((info.id === 3 && index === 9) || (info.id === 3 && index === 10)) {
            if (index === 9) {
                return (
                    <NavLink to="/" id={s["blue"]} key={Math.random() * 1000}>
                        {text}
                    </NavLink>
                );
            } else {
                return (
                    <span
                        className={s.projects__text__tools}
            ...
            return (
                <NavLink to="/" key={Math.random() * 1000}>
                    {text}
                </NavLink>
            );
        }
        ...
        <div className={s.dropdown}>
            <button
                className={`${s.dropdown__button} ${
                    isContentVisible && isOpened ? s.up : ""
                }`}
            ...
            <div
                className={`${s.dropdown__content} ${
                    isContentVisible && isOpened ? s.visible : s.invisible
                }`}
            >
